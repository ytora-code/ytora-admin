...
public final class TimeWheelSchedulerImpl implements ITimeWheelScheduler {
     ...
     */
    private final Queue<Object> newTaskQueue;
        ...
        while (true) {
            ...
            for (; ; ) {
                if (log.isTraceEnabled()) {
                    log.trace("开始新一轮循环，时间轮当前刻度：{},总刻度：{}", currentSlot, mask + 1);
                }
                Object command = newTaskQueue.poll();
                //取不到新任务，则跳出循环
                if (command == null) {
                    break;
                }
                //停止时间轮，结束while true死循环
                else if (command == exitTask) {
                    return;
                }
                else if (command instanceof CancelRequest cancelRequest) {
                    processCancelRequest(cancelRequest);
                }
                //注册新任务
                else {
                    TimeWheelTask newTask = (TimeWheelTask) command;
...
//                    long nextPeriod = nextExecMilli - now;
    ...
    public Boolean cancelTask(String taskId) {
        ...
        CancelRequest cancelRequest = new CancelRequest(task);
        if (!newTaskQueue.offer(cancelRequest)) {
            throw new BaseException("新任务队列满了");
        }
        try {
            cancelRequest.latch.await();
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new BaseException("等待取消任务完成时被中断");
        }
        return cancelRequest.cancelled;
    }

    private void cleanupTask(TimeWheelTask task) {
        String taskId = task.getTaskId();
        if (taskId != null) {
            existTask.remove(taskId, task);
        }
    }

    private void processCancelRequest(CancelRequest cancelRequest) {
        TimeWheelTask task = cancelRequest.task;
        String taskId = task.getTaskId();
        if (taskId == null) {
            cancelRequest.cancelled = false;
            cancelRequest.latch.countDown();
            return;
        }
        if (!existTask.containsKey(taskId)) {
            cancelRequest.cancelled = false;
            cancelRequest.latch.countDown();
            return;
        }
