...
public class LocalFileStorageServiceImpl implements IFileStorageService {
    ...
    public String upload(InputStream is) {
        ...
        // 2) 真正写入文件
        try {
            Files.createDirectories(target.dirAbsNorm());
            Files.copy(is, target.dataFileAbsNorm());
        } catch (FileAlreadyExistsException e) {
            // 检查通过到写入这段极短窗口发生并发冲突：直接报错
            throw new StorageException("写入时发生并发fileId冲突: " + target.fileId(), e);
        } catch (IOException e) {
            // 不要求清理桶目录；文件如果写了一半，deleteIfExists 尽量清理掉
            safeDelete(target.dataFileAbsNorm());
            throw new StorageException(e);
        }

        log.info("文件上传成功，fileId: {}", target.fileId());
    ...
    public InputStream download(@NonNull String fileId) {
        ...
        if (!Files.exists(dataFile)) throw new StorageException(ResultCode.FILE_NOT_FOUND);

        try {
            return Files.newInputStream(dataFile, StandardOpenOption.READ);
        } catch (IOException e) {
            throw new StorageException(ResultCode.FILE_DOWNLOAD_FAIL);
        }
    }
    ...
    public void remove(@NonNull String fileId) {
        ...
        if (!deleted) throw new StorageException(ResultCode.FILE_NOT_FOUND);
    }
    ...
    private void validateFileId(String fileId) {
        if (fileId == null || !fileId.matches("^[A-Za-z0-9\\-]{10,}$")) {
            throw new StorageException(ResultCode.ERROR_FILE_ID);
        }
    }
    ...
    private boolean safeDelete(Path p) {
        try {
            ...
            throw new StorageException(ResultCode.FILE_DELETE_FAIL);
        }
    }
